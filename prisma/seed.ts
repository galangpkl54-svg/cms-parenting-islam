...
import { BannerPosition, CommentStatus, PostStatus, PrismaClient, UserRole, YoutubePosition } from "@prisma/client";
...
async function ensureCategory(data: { name: string; slug: string }) {
...
async function ensureTag(data: { name: string; slug: string }) {
...
async function main() {
  ...
  const posts = await Promise.all(
    demoPosts.map((post) =>
      ensurePost(
        admin.id,
        post,
        post.categories.map((slug) => categoryMap.get(slug)).filter(Boolean) as string[],
        post.tags.map((slug) => tagMap.get(slug)).filter(Boolean) as string[]
      )
    )
  );
  ...
  console.log(`Seeded demo content for ${env.NODE_ENV} as ${adminEmail}`);
}
